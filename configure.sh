...
if [ "$1" = "Android" ]; then
  ...
  echo "********************************************************************************"
  cmake $(dirname $0) -G"$CMAKE_GENERATOR" -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_ECLIPSE_MAKE_ARGUMENTS="-j$NUMBER_OF_PROCESSORS -s" -DCMAKE_TOOLCHAIN_FILE="$ANDROID_TOOLCHAIN" -DLIBRARY_OUTPUT_PATH_ROOT="." -DANDROID_NATIVE_API_LEVEL="$ANDROID_NATIVE_API_LEVEL" -DANDROID_ABI="$ANDROID_ABI"
  echo "********************************************************************************"
