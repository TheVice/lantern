...
void rasterized_triangle_app::initialize(int width, int height)
{
	...
	const std::string resourcePath = "triangle.obj";

	if (mResources.count(resourcePath))
	{
		m_triangle_mesh.reset(new mesh(
			load_mesh_from_obj(mResources[resourcePath], false, false)));
		//
...
std::map<std::string, std::string> rasterized_triangle_app::mResources;
